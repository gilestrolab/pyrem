...
For further investigation, $p=21$ was considered to be a good compromise between error and computational load. There relative importances are presented in table~\ref{tab:importances}.
...
Significant improvement was achieved by both methods by including even little temporal information ($\tau = 1$, $n=\{1,3\}$\TODO{stats}).
Although both methods improve performance, it appears that the inter-recording variablility is lower using convolution method\TODO{stats}.
The most significant difference with the original set of variables was achieved using $n = \{1,3,7,15,31\}$.
