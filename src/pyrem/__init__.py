...
2. A data structure for annotations (:class:`pyrem.time_series.Annotation`) based on numpy recarrays.
    This allows to describe time series of discrete states linked to a confidence/probability of observation of each states.

3. A data structure for collection of time series (:mod:`pyrem.polygram`).
    It features:
...
4. Implementations of algorithms often used in analysis of EEGs (see :mod:`pyrem.univariate`).
    They are essentially  faster and curated reimplementation of the function available in PyEEG_.

5. Utilities to load, save and visualize the data (still in development)

6. Wrappers around samplerate_ and pywt_ libraries, to efficiently resample,
    and compute discrete wavelet decomposition on signals.
