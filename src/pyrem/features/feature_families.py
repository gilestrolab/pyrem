...
class PowerFeatures(FeatureFamily):
    prefix = "power"
...
class NonLinearFeatures(FeatureFamily):
    prefix = "nl"
    def _make_feature_vec(self,channel):
        data = channel.data.flatten()
        out = dict()
...
class HjorthFeatures(FeatureFamily):
    prefix = "hjorth"
    def _make_feature_vec(self, channel):
        data = channel.data.flatten()
        a,m,c = hjorth(data)
...
class EntropyFeatures(FeatureFamily):
    prefix = "entropy"
...
class WaveletsFeaturesBase(FeatureFamily):
    prefix = "wavelets"
    _max_level = 5
    _wavelet =  None
    def __init__(self):
...
class PeriodFeatures(FeatureFamily):
    prefix = "welch"
